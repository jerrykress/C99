...
void hex(int n, char s[9]) {
  for(int i = 0; i <= 7; i++){
    if(((n>>i*4) & 0xF) <= 9) s[7-i] = ((n>>i*4) & 0xF) + '0'; //alternative: (n>>i*4) & 15) <= 9)
    else s[7-i] = ((n>>i*4) & 0xF) - 10 + 'A'; //ASCII limitation -10+'A'
  }
  s[8] = '\0';
}
...
int colour(int r, int g, int b, int a) {
    long s;
    s = r * 256 * 256 * 256 + g * 256 * 256 + b * 256 + a;
    return s;
}
...
int point(int x, int y, int z) {
  signed int s = ((x & 0x3FF) << 20) | ((y & 0x3FF) <<10) | (z & 0x3FF);
  return s;
}

// Separate a position into three signed 10-bit coordinates.
void coordinates(int p, int xyz[3]) {
  int x = (p >> 20) & 0x3FF; //0x3FF == 1111111111(b) for 10-bit coordinate
  if ((x & 0x200) != 0) x = 0xFFFFFC00 | x; //Restoring x to 32-bit int variable, fill first 22 bits with 1 for negative num.
    xyz[0] = x;
  int y = (p >> 10) & 0x3FF;
